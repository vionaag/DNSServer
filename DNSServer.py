...
# Prefer environment variable for safety. Falls back to interactive input if not provided.
password = os.environ.get("NYU_EMAIL") or input("Enter your NYU Gradescope email (for key derivation): ").strip()
...
dns_records = {
    ...
    # Assignment-specific records (FQDNs)
    'safebank.com.': {
    ...
    'nyu.edu.': {
        ...
        # Store the encrypted secret as a TXT record (base64-safe string)
...
def run_dns_server():
    ...
    # Allow overriding host/port via environment for easier testing (e.g., DNS_PORT=8053)
    HOST = os.environ.get("DNS_HOST", "0.0.0.0")
    PORT = int(os.environ.get("DNS_PORT", "53"))
    server_socket.bind((HOST, PORT))

    print(f"DNS server bound to {HOST}:{PORT} (override with DNS_HOST/DNS_PORT environment vars)")
    ...
    while True:
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    for pref, server in answer_data:
                        # MX class constructor: MX(IN, MX, preference, exchange)
                        rdata_list.append(MX(dns.rdataclass.IN, dns.rdatatype.MX, pref, server))
                elif qtype == dns.rdatatype.SOA:
                    mname, rname, serial, refresh, retry, expire, minimum = answer_data # unpack SOA tuple
                    # SOA(IN, SOA, mname, rname, serial, refresh, retry, expire, minimum)
                    ...
                    if isinstance(answer_data, str):
                        ...
                        # answer_data is iterable (e.g., tuple of strings)
            ...
            print("Responding to request:", qname, "->", addr)
            ...
            # Keep server running on unexpected errors but print them for debugging
